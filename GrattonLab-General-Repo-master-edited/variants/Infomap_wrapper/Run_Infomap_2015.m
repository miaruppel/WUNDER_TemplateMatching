...
for i = 1:numanalyses
    if i<numanalyses
        ...
        disp(['Threshold ' num2str(thresholdarray(i)) ': writing ' num2str(edgesleft) ' edges']);
...
if isempty(gcp('nocreate'))
    parpool(8);  % start the pool if it's not already open
end

addAttachedFiles(gcp, '/data/smyser/smyser4/wunder/wunder_caf_III/TemplateMatching/GrattonLab-General-Repo-master/variants/filenamefinder.m');  % attach the needed function
...
%     fprintf('Thr/box %d, pass %d\n', i);
%     tic
%     pajekfile = fullfile(outdir, ['pajek_col' num2str(i) '.net']);
%     rawclrs = run_infomap_on_pajekfile(pajekfile, 100);
%     dlmwrite(fullfile(outdir, ['rawassn_col' num2str(i) '.txt']), rawclrs, '\t');
%     toc
% end
