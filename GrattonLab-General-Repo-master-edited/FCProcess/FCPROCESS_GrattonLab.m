function FCPROCESS_GrattonLab(datafile, outputDir, FCPROCESS_params_file, varargin)
...
[paramspath,paramsname,paramsextension] = fileparts(FCPROCESS_params_file);
origpath = pwd;
...
%         cd(QC(i).subbolddir{j});
        if QC(i).residuals ~= 0
            all_fstring = sprintf('sub-%s_ses-%d_task-%s_residuals_run-%d',QC(i).subjectID,QC(i).session,QC(i).condition,QC(i).runs(j));
        ...
        if ~exist(tboldmot_folder{i,j}, 'dir')
...
for i=1:numdatas
    dfnd_name_out = [QC(i).sessdir_out QC(i).naming_str{1}(1:end-6) '_desc-AllRunUnionMask.nii.gz'];
...
switch switches.regressiontype 
    case {0,1,9} % freesurfer masks of WM and V
        ...
        for i=1:numdatas
            ...
            QC(i).GLMmaskfile = [templateflow_dir '/tpl-' space '/tpl-' space '_res-02_desc-brain_mask_dilate3.nii.gz']; %CG = primary mask we will use
            
            % need to resample the maskfiles to res02 space 
            fnames = resample_masks(anat_string,QC(i),space,project_dir,singularity_cmd_start,afni_sif);
...
function fnames = resample_masks(anat_string,QC,space,project_dir,singularity_cmd_start,afni_sif)
