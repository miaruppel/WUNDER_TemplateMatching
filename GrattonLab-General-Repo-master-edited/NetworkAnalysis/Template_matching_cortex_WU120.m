function Template_matching_cortex_WU120(data_info, outDir, FSUorNUorWU)
...
if ~exist('FSUorNUorWU', 'var')
    error('Must specify ''FSU'' or ''NU'' or ''WU''as input to set up correct paths');
end
...
%end
	...
	%end
        ...
        for r = 1:length(QC.runs) % loop through runs (may not be continuous)
            ...
            %cifti_fstring = sprintf('%s/sub-%s/ses-None/cifti_timeseries_normalwall/sub-%s_ses-%d_task-rest_run-%d_LR_surf_subcort_222_32k_fsLR_smooth2.55.dtseries.nii',cifti_dirs{s},QC(i).subjectID,QC(i).session,QC(i).subjectID,QC(i).session,QC(i).runs(r));
            ...
            tmask_fname = [conf_fstring1 'DCANBOLDProc_v4.0.0/analyses_v2/motion/' all_fstring2 '-tmask_' QC.FDtype '.txt']; %assume this is in confounds folder
    ...
    %%% write out results %%%
    dice_match_fname = sprintf('%s/sub-%s_dice_to_templates.mat',outDir,QC.subjectID);
    save(dice_match_fname, 'dice_to_templates','-v7.3');
    dice_map_fname = sprintf('%s/sub-%s_dice_WTA_map_kden0.05.dtseries.nii',outDir,QC.subjectID);
