function Run_Infomap(rmat, dmatname, xdistance, thresholdarray, makebinary, outdir, numpools, structure_indices)
...
if ~isempty(xdistance)
    if ~isequal(size(dmat), size(rmat))
        error('distance matrix and correlation matrix dimensions do not match');
    elseif isnumeric(xdistance) && (xdistance>=0)
...
%eval(['matlabpool open ' num2str(numpools)]) % CG - turned this off for
%now
pool=parpool(numpools); %2/8/22 added back in
